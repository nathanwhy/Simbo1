...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self setBackgroundColor:kGlobalBg];
        
        _scrollView=[[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, 320, 216)];
        ...
        [_pageControl setCurrentPageIndicatorTintColor:kMainColor];
        [_pageControl addTarget:self action:@selector(pageChange) forControlEvents:UIControlEventValueChanged];
        [self addSubview:_pageControl];
        
        NSMutableArray *array = [NSMutableArray arrayWithCapacity:471];
        int pagecount = 471/27+1;
        int tag = 0;
        for (NSInteger page = 0; page < pagecount; page++) {
            for (NSInteger row = 0; row < kRowCount; row++) {
                for (NSInteger col = 0; col < kColCount; col++) {
                    ...
                    [button setFrame:CGRectMake(x + page*320, y, kButtonSize.width, kButtonSize.height)];
                    button.tag = tag;
...
-(void)layoutSubviews
{
    ...
    CGRect frame;
    frame.size = CGSizeMake(self.frame.size.width, 20);
    frame.origin.x = 0;
...
{
    return [NSString stringWithFormat:@"%C", emotechar[index]];
}

-(void)pageChange
{
    NSInteger page = _pageControl.currentPage;
    float x = self.frame.size.width * page;
...
{
    long index = button.tag - button.tag/28;
...
{
    int page=scrollView1.contentOffset.x/self.frame.size.width;
